  ...
  //cadastro fabricante :
         ...
         //cadastro dos fabricantes:
            ...
            for(int con=0; con < qnt_produto;con++){//incio for: 
                ...
                printf("\nInforme o peso do produto e a sua medida (por exemplo '45kg'): ");
                scanf(" %s", &produto[con].peso);
...
void Opcao2(struct Produto* produto, int qnt_produto){
  ...
  for(int i=0; i<qnt_produto;i++){
    printf("- Nome: %s | Peso: %s | Valor: %.2f | Fabricante: %s\n", produto[i].descricao, produto[i].peso, produto[i].venda, produto[i].fabricante);
    }
}//fim função
